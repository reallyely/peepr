...
export class WorkItemIntegration {
  readonly id: number;
  ...
  static create(data: {
    ...
    try {
      ...
      const mergedAt = data.mergedAt ?
        (data.mergedAt instanceof Date ? data.mergedAt : new Date(data.mergedAt)) :
        null;

      const closedAt = data.closedAt ?
        (data.closedAt instanceof Date ? data.closedAt : new Date(data.closedAt)) :
        null;

      // Convert string durations to Duration objects if needed
      const prTimeOpen = data.prTimeOpen instanceof Duration ?
        data.prTimeOpen :
        new Duration(typeof data.prTimeOpen === 'string' ?
          Duration.fromHumanReadable(data.prTimeOpen).inMilliseconds :
          0);

      const totalDuration = data.totalDuration instanceof Duration ?
        data.totalDuration :
        new Duration(typeof data.totalDuration === 'string' ?
          Duration.fromHumanReadable(data.totalDuration).inMilliseconds :
          0);

      return new WorkItemIntegration(
        data.id ?? Math.random(),
        data.prNumber,
        data.title,
        createdAt,
