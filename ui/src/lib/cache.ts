...
export const Cache = {
  ...
  async set(key: string, data: unknown, ttl?: number): Promise<void> {
    try {
      ...
      const expiry = ttl || TTL_CONFIG[keyPrefix as keyof typeof TTL_CONFIG] || DEFAULT_TTL;
   ...
   */
  async getOrSet<T>(key: string, dataFn: () => Promise<T>, ttl?: number, forceFresh = false): Promise<T> {
    if (!forceFresh) {
